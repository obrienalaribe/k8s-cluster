...
resource "null_resource" "run_kong_ansible_playbook" {
...
resource "null_resource" "run_rook_ansible_playbook" {
  ...
  provisioner "local-exec" {
    command = "export PATH=$PATH:$HOME/.local/bin ; export K8S_AUTH_KUBECONFIG=${abspath(local_file.kubeconfig.filename)} ; ansible-playbook ansible/playbooks/install-rook.yml"
  }
  triggers = {
    build_number = timestamp()
  }
